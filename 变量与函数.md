...
3. 高阶函数
    ...
    并且，如果高阶函数参数中的函数，只有一个参数，那么小括号也可以省略：
...
9. 运算符重载
    ...
    实际上，以Int为代表的标准库的数值类型，他们的运算操作也只不过是方法调用，比如`1 + 1`等价于`1.+(1)`。

    比kotlin更强大的是，kotlin是规定我们可以重载什么运算符，而scala是除了`=`以外的少数符号不能重载以外，其他的各种符号都可以当做方法名，并返回任意的类型，比如我们可以重载kotlin不能重载的`>=`、`&&`等符号。
    ...
    另外，scala中还有一类行为特殊的运算符，如果方法名中包含冒号，那么，方法的接收者是右侧的值，而不是左侧的值：
