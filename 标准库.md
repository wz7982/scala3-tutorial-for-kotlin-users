...
3. Option
    ...
    kotlin使用`?:`运算符来给一个一个可空类型附加默认值，并抛弃可空性：
    ...
    如果你喜欢kotlin的空值处理风格，那我们说过，scala是允许把符号作为函数名的，只需要加一些扩展方法就可以在“库层面”添加这些运算符，只是扩展的时候千万别忘了`?:`的接收者在右侧哦。（!!这种运算符为了省去方法调用中的点，可以import scala.language.postfixOps）。
