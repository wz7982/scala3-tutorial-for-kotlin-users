...
3. Option
    ...
    如果你喜欢kotlin的空值处理风格，那我们知道，scala是允许把符号作为函数名的，只需要加一些扩展方法就可以在**库层面**添加这些运算符，而无需在**语法级别**支持这些。
    ...
    看起来非常不错，但是`!!`前面的点有点不舒服，我们导入`import scala.language.postfixOps`来去掉这个点，`postfixOps`允许无参的方法省略点：
    ...
    在scala里，只需要几个普通的扩展方法就可以模拟kotlin的核心卖点之一。
...
4. Tuple
    ...
    如果我们把元组和列表结合起来看，会发现一件事，们它的构造很像，如果列表里面的数据类型是同质的话，那么元组的元素就是异质的（不同类型），我们一般管元组这样的结构叫做hlist（异构列表），那么它自然应该有类似列表的构造方式：
